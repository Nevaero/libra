...
`settlement` (T+2 scheduling and batch), `trading` (order orchestrator), and `api` (thin REST
controllers over the module ports). WebSocket streaming is planned.
...
- **API reference (OpenAPI)**: generated by springdoc from the controllers and DTOs. With the app
  running, the spec is at `/v3/api-docs` and Swagger UI at `/swagger-ui.html`.
...
The REST API is in place (no authentication yet); WebSocket streaming and a frontend are planned.
...
(orchestrateur d'ordres), et `api` (controllers REST thin au-dessus des ports). Le streaming
WebSocket est prévu.
...
- **Référence API (OpenAPI)** : générée par springdoc à partir des controllers et des DTOs. App
  démarrée, le spec est à `/v3/api-docs` et Swagger UI à `/swagger-ui.html`.
...
niveau des frontières. L'API REST est en place (sans authentification pour l'instant) ; le streaming
WebSocket et un frontend sont prévus.
